...
def main():
    ...
    for i in range(0, 21):
        # Write to CSV file
        rows = 50
        file_name = f"test_data/input{i:02d}.csv" if i != 0 else "test_data/input.csv"
        with open(file_name, "w", newline="") as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow(["virus", "human"])
            for j in range(rows):
                virus_sequence = generate_random_dna(15) * 10
                human_sequence = generate_human_dna(virus_sequence, 200, 4000)
